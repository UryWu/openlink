#!/bin/bash
# scripts/bump_version.sh — bump backend / frontend / extension versions
#
# Each component has its own version slot. By default all three move together.
# Pass --backend / --frontend / --extension to override individual components
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --extension) OVERRIDE_EXTENSION="$2"; shift 2 ;;
    -h|--help)
...
for pair in "backend:$OVERRIDE_BACKEND" "frontend:$OVERRIDE_FRONTEND" "extension:$OVERRIDE_EXTENSION"; do
  comp="${pair%%:*}"; val="${pair##*:}"
...
for comp in backend frontend extension; do
